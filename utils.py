...
class BleuScore:
    ...
    def compute_blue_score(self, real, predicted):
        ...
        for sent1, sent2 in zip(real, predicted):
            ...
            score.append(
                sentence_bleu(
                    [sent1], sent2, weights=(self.w1, self.w2, self.w3, self.w4)
                )
            )
        return score
...
def create_masks(src, trg, padding_idx):
    src_mask = (
        (src == padding_idx).unsqueeze(-2).type(torch.FloatTensor)
    )  # [batch, 1, seq_len]

    trg_mask = (
        (trg == padding_idx).unsqueeze(-2).type(torch.FloatTensor)
    )  # [batch, 1, seq_len]
